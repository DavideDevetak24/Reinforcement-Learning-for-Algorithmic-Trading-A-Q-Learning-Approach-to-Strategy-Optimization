...
training <- function(j, Q_table, df_train, actions, alpha, gamma, epsilon, min_epsilon, epsilon_decay) {
  ...
  for (i in 1:(nrow(df_train_local) - 1)) {
    ...
    if (runif(1) < epsilon) { 
      ...
      action <- actions[which.max(Q_table_local[J(current_state), ..actions])]
    }

    reward <- reward_function(df_train_local[i, ])
    best_future_q <- max(Q_table_local[J(next_state), ..actions], na.rm = TRUE)

    Q_table_local[J(current_state), (action) := (1 - alpha) * get(action) + alpha * (reward + gamma * best_future_q)]
    
    epsilon <- max(min_epsilon, epsilon * epsilon_decay)
  }

  df_train_local$RL_Position <- 0  
  for (i in 1:(nrow(df_train_local) - 1)) {
    state <- df_train_local$State[i]
    action <- actions[which.max(Q_table_local[J(state), ..actions])]
    df_train_local$RL_Position[i] <- ifelse(action == "Long", 1, ifelse(action == "Short", -1, 0))
  }

  df_train_local$RL_strategy_return <- df_train_local$daily_return * df_train_local$RL_Position
  df_train_local$cumulative_RL_return <- cumsum(df_train_local$RL_strategy_return)
  df_train_local$cumulative_RL_return <- 1 + (df_train_local$cumulative_RL_return - df_train_local$cumulative_RL_return[1])

  return(data.frame(date = df_train_local$date, cumulative_RL_return = df_train_local$cumulative_RL_return))
}
```
...
clusterExport(cl, varlist = c("df_train", "actions", "Q_table", "reward_function", 
                              "alpha", "gamma", "epsilon", "min_epsilon", "epsilon_decay"), 
              envir = environment())

start_time <- Sys.time()

results <- foreach(j = 1:iter, .packages = c("data.table"), 
